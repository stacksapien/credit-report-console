...
import AgentLoanRequest from '../../components/agentLoanRequest';
...
class AgentLoanRequests extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        serviceGet(`api/v1/agent/${this.state.agent.id}/user/${this.state.userId}/loan-requests`, headers)
        .then((res) => {
            if(res.data){
                this.setState({
                    offerData: res.data.requestData,
    ...
    submitDetails = (event) => {
        ...
        let loanRequest = {
            ...
            agentType: this.state.agent.role[0]
        }
        ...
        servicePost(`api/v1/agent/${this.state.agent.id}/loan-request`, loanRequest, headers)
        .then((res) => {
            if(res.data){
                console.log("data");
                let updatedRequestsData = [res.data, ...this.state.offerData];
                this.setState({
                    offerData: updatedRequestsData,
                    loading: false
                })
                                        ...
                                        <React.Fragment>
                                            ...
                                            <br/><br/>
                                            {
                                                this.state.offerData.map((data) => (
                                                    <AgentLoanRequest key={data._id} data={data} user={this.state.userData} agent={this.state.agent} deleteRequest={this.deleteLoanRequest}/>
                                                 ))
                                            }
                                        </React.Fragment>
                                    )}
...
export default withRouter(AgentLoanRequests);
